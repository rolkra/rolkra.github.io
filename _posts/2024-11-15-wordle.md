...
So, instead of just using words from a dictionary, I decided to start playing and simply collecting the data (but not using them).
...
My advantage seems to be gone. But I continued collecting data, hoping that they will give me a boost again.
...
OK, so it seems to be obvious, that using a "start-word" that contains a lot of "popular" letters is helpful. And in case no letter matches, 
...
Using (even small data) can give you an big advantage that outperformes using logic and intuiation.
